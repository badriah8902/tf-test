...
public class BlogResource {
    ...
    /**
     * {@code DELETE  /blogs/clean} : delete all blog with keyword
     *
     * @param keyword the keyword of the blogDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/blogs/clean")
    public ResponseEntity<Void> deleteAllEntryWithKeyword( @RequestParam String keyword) {
        log.debug("REST request to delete entry with these Keyword : {}");

        Long id = entry.getId();

        if (entry.getTitle().contains(keyword) == true || entry.getContent().contains(keyword) == true ) {
     ...
     /**
     * {@code DELETE  /blogs/{id}/clean} : delete entry for keyword
     *
     * @param keyword the keyword of the blogDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 204 (NO_CONTENT)}.
     */
    @DeleteMapping("/blogs/{id}/clean")
    public ResponseEntity<Void> deleteEntryWithKeyword(@PathVariable Long id, @RequestParam String keyword) {
        log.debug("REST request to delete entry with these Keyword : {}", id);
